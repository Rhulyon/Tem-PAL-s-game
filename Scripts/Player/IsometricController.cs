...
public abstract class IsometricController : MonoBehaviour {
    ...
    {
        colliders = Physics2D.OverlapCircleAll(transform.position, transform.localScale.y / 2 + 0.05f);
        ...
        foreach(Collider2D collider in colliders)
        {
            switch (collider.tag)
            {
                ...
                case "Door":
                    ...
                    KeyDoor door = collider.GetComponent<KeyDoor>();
                    if (door != null)
                    {
                        if (hasKey)
                            door.OperateDoor();
                    ...
                    break;
                case "Key":
                    Debug.Log("You picked up the key, you can now unlock doors.");
