            ...
            switch (faceDir)
                {
                    ...
                    case PlayerFaceDirection.Front:
                        aux = CarToIso(Vector2.up * (transform.localScale.y / 2 + currentBoulder.transform.localScale.y / 2));

                        break;
                    case PlayerFaceDirection.Back:
                         aux = CarToIso(Vector2.down * (transform.localScale.y / 2 + currentBoulder.transform.localScale.y / 2));
                        break;
                    case PlayerFaceDirection.Left:
                         aux = CarToIso(Vector2.left * (transform.localScale.y / 2 + currentBoulder.transform.localScale.y / 2));
                        break;
                    case PlayerFaceDirection.Right:
                         aux = CarToIso(Vector2.right * (transform.localScale.y / 2 + currentBoulder.transform.localScale.y / 2));
                        break;
                }
                currentBoulder.transform.position = aux;
